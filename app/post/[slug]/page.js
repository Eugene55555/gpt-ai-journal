...
const ptComponents = {
  block: {
    h2: ({ children }) => {
      ...
      return <h2 id={id} className="scroll-mt-32 text-[32px] font-medium mt-20 mb-6 text-gray-900 tracking-tight">{children}</h2>;
    },
    h3: ({ children }) => {
      const id = slugify(children.toString());
      return <h3 id={id} className="scroll-mt-32 text-[24px] font-medium mt-12 mb-4 text-gray-900 tracking-tight">{children}</h3>;
    },
    normal: ({ children }) => <p className="mb-8 leading-[1.8] text-[18px] text-gray-800 font-light">{children}</p>,
  },
  types: {
    image: ({ value }) => {
      ...
      return (
        <figure className="my-16">
          <div className="relative aspect-video rounded-2xl overflow-hidden border border-gray-100 bg-gray-50 shadow-sm">
            <Image src={value.url} alt={value.alt || "Изображение к статье"} fill className="object-cover" />
          </div>
          {value.caption && <figcaption className="text-center text-[13px] text-gray-400 mt-5 font-light tracking-wide italic">{value.caption}</figcaption>}
        </figure>
      );
    },
    blockquote: ({ value }) => (
      <blockquote className="border-l-2 border-black pl-8 my-16 py-2">
        <p className="text-[24px] font-light italic leading-relaxed text-gray-900">{value.text}</p>
        {value.author && <cite className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-400 mt-6 block not-italic">— {value.author}</cite>}
      </blockquote>
    ),
    inlineFile: ({ value }) => {
      if (!value?.url) return null;
      return (
        <div className="my-12">
          <a href={`${value.url}?dl=`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-between p-6 rounded-2xl border border-gray-100 bg-gray-50 hover:bg-white hover:border-gray-200 hover:shadow-xl hover:shadow-gray-100 transition-all duration-300 group no-underline">
            <div className="flex items-center gap-5">
              ...
              <span className="text-[15px] font-medium text-gray-900 group-hover:text-black transition-colors">{value.buttonText || "Скачать файл"}</span>
            </div>
            <span className="hidden sm:inline text-[10px] font-bold text-gray-400 uppercase tracking-widest">PDF / DOCX</span>
          </a>
        </div>
      );
    },
...
export default async function PostPage({ params }) {
  ...
  // ДОБАВЛЕНО: Получаем ID категории и её имя
  const post = await client.fetch(
    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-md border-b border-gray-100 transition-all duration-300">
        <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <Link href="/" className="group flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 hover:text-black transition-colors">
      ...
      <main className="max-w-[1400px] mx-auto px-6 pt-32 pb-32 animate-fadeIn">
        <div className="flex flex-col lg:flex-row gap-20 items-start">
          <div className="flex-grow lg:max-w-[800px] w-full">
            ...
            <Breadcrumbs categoryValue={post.categoryValue} categoryName={post.categoryName} title={post.title} />

            <header className="mb-16">
              <div className="flex items-center gap-4 mb-8">
                <span className="text-[11px] font-bold uppercase tracking-[0.2em] px-3 py-1 bg-gray-100 rounded-full text-gray-600">
                  {post.categoryName || "БЕЗ КАТЕГОРИИ"}
                </span>
              ...
              <div className="flex items-center gap-5 py-8 border-y border-gray-100">
                <div className="relative w-12 h-12 rounded-full overflow-hidden bg-gray-50 border border-gray-100 shrink-0 shadow-sm">
                  {post.author?.avatarUrl ? (
                    ...
                    <div className="flex items-center justify-center h-full text-sm font-bold text-gray-300">A</div>
                  )}
                </div>
                <div>
            ...
            {post.fileUrl && (
              <div className="mt-20 pt-10 border-t border-gray-100">
                <h4 className="text-[11px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-6">Дополнительные материалы</h4>
                <a 
            ...
            {relatedPosts.length > 0 && (
              <div className="mt-32 pt-16 border-t-2 border-black">
                <h3 className="text-[32px] font-medium tracking-tightest mb-10">Читайте также</h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {relatedPosts.map((rp) => (
                    <Link key={rp._id} href={`/post/${rp.slug}`} className="group block">
                      <div className="aspect-[16/10] bg-gray-50 rounded-2xl overflow-hidden mb-5 relative border border-gray-100 shadow-sm">
                        {rp.imageUrl ? (
                          <Image src={rp.imageUrl} alt={rp.title} fill className="object-cover transition-transform duration-700 ease-out group-hover:scale-105" />
                        ) : (
                      ...
                      <div className="space-y-3">
                        <span className="text-[10px] text-gray-400 uppercase font-medium tracking-widest block">
                          {new Date(rp.publishedAt).toLocaleDateString("ru-RU", { day: 'numeric', month: 'long' })}
                        </span>
