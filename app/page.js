...
export default async function Page({ searchParams }) {
  ...
  const data = await client.fetch(query, queryParams, { next: { revalidate: 0 } });
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-100">
        <div className="max-w-[1400px] mx-auto px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-10">
            <Link href="/" className="text-[22px] font-bold tracking-tighter hover:opacity-70 transition-opacity">
              {siteConfig.name}
            </Link>
      ...
      <main className="flex-grow max-w-[1400px] mx-auto px-6 pt-32 pb-20 w-full animate-fadeIn">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-10 pb-12 border-b border-gray-100">
            <div className="max-w-3xl">
              <h1 className="text-[56px] md:text-[84px] font-medium tracking-tightest leading-[0.9] mb-8 text-black whitespace-pre-line">
                {category ? (
                  siteConfig.navigation.find((n) => (n.value || "") === category)?.name || category
                ) : (
              ...
              <p className="text-[20px] text-gray-500 font-light max-w-[500px] leading-relaxed">
                {category 
                  ? `Публикации в разделе «${siteConfig.navigation.find((n) => (n.value || "") === category)?.name || category}»` 
        ...
        {posts.length === 0 ? (
          <div className="py-24 text-center">
            <p className="text-gray-400 text-[18px]">Ничего не найдено по вашему запросу.</p>
            <Link href="/" className="text-black underline mt-4 inline-block font-medium">Сбросить фильтры</Link>
          </div>
        ) : (
          <>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] text-gray-400 uppercase font-medium">
                        {new Date(post.publishedAt).toLocaleDateString("ru-RU", { day: 'numeric', month: 'short' })}
                      </span>
                    </div>
